...
Some strategies for improving MCMC
 ...
 - Reparameterize
    ...
    - Centered versus non-centered random effects parameterization [discussed here]
    - Rotated coordinates to reduce posterior correlation [already seen]
 - Rewrite the model. E.g.,
    - Rewrite the model to reduce dependencies 
    - Vectorize declarations to improve computational efficiency
    - Marginalize to remove parameters [shown here]
...
![MCMC efficiency (estimated by the `mcmcse` method) for parameters](main2_efficiencySummary_mcmcse.jpg)

And here for the random effects.

![MCMC efficiency (estimated by the `mcmcse` method) for random effects](main2_REs_efficiencySummary_mcmcse.jpg)
