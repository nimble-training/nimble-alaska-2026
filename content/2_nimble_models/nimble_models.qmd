...
- Distributions with alternative parameterizations are listed in [Table 5.2 of User Manual Section 5.2.4](https://r-nimble.org/manual/cha-writing-models.html#tab:parameterizations)
...
See our [guide online](https://r-nimble.org/blog/quick-guide-for-converting-from-jags-or-bugs-to-nimble.html).
...
WARNING: there are no other valid uses of `if` statements in a nimble model (i.e., no run-time use)
...
layout <- matrix(ncol = 2, byrow = TRUE,
   ...
   # so I'll just use 0-100 as the scale
                 data = c(33, 100,
                          ...
                          20, 75, 40, 75, 60, 75, 80, 75, # predicted_y's
...
- E.g., `predicted_y[2]` comes before `y[2]`.
...
code2 <- nimbleCode({
  ...
  for(i in 1:4) {
    # predicted_y[i] <- intercept + slope * x[i] # scalar nodes (earlier model version)
    y[i] ~ dnorm(predicted_y[i], sd = sigma)
  }
})
...
code2_macros <- nimbleCode({
  sigma ~ dunif(0, 100)
  predicted_y[1:4] <- LINPRED(~x)  
  for(i in 1:4) {
    y[i] ~ dnorm(predicted_y[i], sd = sigma)
  }
})

model2_macros <- nimbleModel(code2_macros, 
                      data = list(y = rnorm(4)),
                      inits = list(beta_Intercept = 0.5,  # See docs for naming (or run once)
                                   beta_x = 0.2,
                                   x = c(0.1, 0.2, 0.3, 0.4)))

model2_macros$getCode()
model2_macros$getNodeNames()
```
